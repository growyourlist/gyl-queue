					...
					return {
						promise: () =>
							new Promise((resolve) => {
								...
								console.log(
									`Sending "${input.Template}" template to: ${list
										.slice(0, 2)
										.join(', ')}${list.length > 2 ? '...' : ''}`
								);
					...
					return {
						promise: () =>
							new Promise((resolve) => {
								...
								console.log(
									`Sending ${
										messageKeys.length ? messageKeys.join(' and ') : 'blank'
									} email to ${input.Destination.ToAddresses[0]}`
								);
...
if (!process.env.SOURCE_EMAIL || !process.env.SOURCE_EMAIL.trim()) {
	throw new Error(
		`Please provide a default SOURCE_EMAIL address in the .env file. This address will be the default from address for all emails that do not have a more specific source email address (i.e. set for the template or list).`
	);
}
	...
	if (itemBody.html) {
		Body.Html = {
			Data: itemBody.html
				.replace(
					/\{\{subscriberId\}\}/g,
					encodeURIComponent(subscriber.subscriberId)
				)
...
const sendWithSes = async (batch, dateStamp) => {
		...
		bulkSends.push(
			new Promise((resolve) =>
				Promise.all([
					...
					const listSourceEmail =
						(templateAndListSettings[1] &&
							templateAndListSettings[1].sourceEmail) ||
						'';
					const unsubscribeLink =
						templateSettings.unsubscribeLink || unsubLinkTemplate;
					try {
						const sesRes = await ses
							.sendBulkTemplatedEmail({
								Source:
									templateSettings.sourceEmail ||
									listSourceEmail ||
							...
							.promise();
						resolve(
							templateGroups[templateId].map((item, index) => {
						...
						resolve(
							templateGroups[templateId].map((item) => ({
								item,
								status: 'failed',
								timestamp: Date.now(),
	...
	textHtmlEmails.forEach((item) => {
		individualSends.push(
			getListSettings(item.tagReason).then(async (listSettings) => {
				// It is just an autoresponder step without an email template
				if (!item.body && item.autoresponderId) {
					return (summary = {
						item,
						status: 'success',
						timestamp: Date.now(),
